...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	// Get identity
	t.identity, err = cid.New(stub)
	if err != nil {
		return shim.Error("Cannot determine caller's identity")
	}
...
func (t *SimpleChaincode) initMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	callerMsp, err := t.identity.GetMSPID()
	if err != nil {
		return shim.Error("Caller MSP ID cannot be determined")
	}
	...
	objectType := "marble"
	marble := &marble{objectType, marbleName, color, size, owner, callerMsp}
...
func (t *SimpleChaincode) delete(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if marbleJSON.OwnerMsp != callerMsp {
		return shim.Error("You cannot manipulate the marble")
	}
...
func (t *SimpleChaincode) transferMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	callerMsp, err := t.identity.GetMSPID()
	if marbleToTransfer.OwnerMsp != callerMsp {
		return shim.Error("You cannot manipulate the marble")
	}
